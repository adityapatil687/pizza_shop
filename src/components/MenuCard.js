...
const MenuCard = ({ pizzaMenu, showToastMessage }) => {
  ...
  useEffect(() => {
    if (selectedSize == "Small") {
      ...
      setPrice(pizzaMenu.price + 20);
    } else if (selectedSize == "Large") {
      setPrice(pizzaMenu.price + 40);
    } else if (selectedSize == "Extra Large") {
      setPrice(pizzaMenu.price + 60);
    }
  }, [selectedSize]);
  ...
  const addToCartHandler = () => {
    ...
    const pizzaToAdd = {
      id: pizzaMenu.id,
      name: pizzaMenu.name,
      price: PizzaPrice,  // variable
      url: pizzaMenu.url,
      size: selectedSize, // variable
    };

    // Check if the pizza with the same id and size already exists in cartData
    const existingPizzaIndex = cartData.findIndex(
      (item) => item.id === pizzaToAdd.id && item.size === pizzaToAdd.size
    );
  ...
  return (
    <div className="col-lg-4  text-center  ">
      <div className="mb-5 border rounded bg-light-subtle">
        ...
        <button
          type="button"
          className="btn btn-success col-10 py-2 mb-3"
          onClick={() => {
            addToCartHandler();
          }}
        >
